...
const GetShipment = ({getModel ,setGetModel ,getShipments  }) => {
    const [index , setIndex] = useState(0)
    const [singleShipmentData , setSingleShipmentData] = useState()

    const getShipmentData = async ()=>{
        const getData = await getShipments(index);
                    ...
                    <div className="max-w-sm mx-auto py-3 space-y-3 text-center">
                        ...
                        <form onSubmit={(e)=> e.preventDefault()} >
                            <div className="relative mt-3 ">
                                <input type="number" placeholder="Enter the id" className="w-full pl-5 pr-3 py-2 text-gray-500 bg-transparent outline-none border
                                focus:border-indigo-600 shadow-sm rounded-lg 
                                " onChange={(e)=> setIndex(e.target.value)} />
                            </div>
                            <button onClick={()=>getShipmentData()} className="block w-full mt-3 py-3 px-4 font-medium text-sm text-center
                        ...
                        </form>
                        {
                            singleShipmentData == undefined ? (
                                <div className="text-gray-400 text-lg " >No Data present </div>
                            ):(
                              <div className="text-left">
                                ...
                                <p>Deliver Time : {convertTime(singleShipmentData.deliveryTime)}</p>
